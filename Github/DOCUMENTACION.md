...
| **2.8** | **Fix de jank y optimización de rendimiento**: (1) **watchdog.js `nuke()`**: intervalo subido de 350ms a 2000ms (5.7x menos carga DOM). (2) **`pruneAds()`**: límite de profundidad (12 niveles) para evitar recursión profunda en JSONs grandes de YouTube. (3) **`pruneGlobals()`**: flag `globalsPruned` para evitar re-podar en cada llamada. (4) **`playbackWatcher`**: subido de 2s a 5s (evita duplicación con el watchdog que ya reporta vía JamasBridge cada 2s). (5) **`memoryMonitorRunnable`**: subido de 10s a 30s (reduce GC forzado que causaba pausas de 1.4s). (6) **`urlWatcher`**: subido de 500ms a 1000ms. (7) **`bgKeepAliveRunnable`**: subido de 3s a 5s. **Resultado en dispositivo real**: Dalvik Heap 95% (248MB/256MB) → 86% (129MB/150MB), frames skipped 86 (~1.4s) → 0, GC forzado cada 10s con pausa 1.4s → natural sin forzar, cero errores. |
...
- **Mini-burbuja**: desliza el vídeo hacia abajo mientras se reproduce y se abre la burbuja flotante (WebView embebida) para seguir viendo/oyendo mientras navegas; ✕ cierra, ⤢ expande a pantalla completa. Si el gesto no se activa en algún dispositivo, sal de la app con el vídeo sonando: como la WebView ya no se pausa, el audio continúa en segundo plano.
